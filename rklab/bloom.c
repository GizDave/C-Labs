...
//returns the position, not index
int
 ...
 *
 * Obtain BLOOM_HASH_NUM bitmap positions by feeding elm to the given hash_i() function  
 *
 * set each position (modulo bitmap size) to 1
 *
	...
	long long elm /* the element to be added (a RK hash value) */)
{
	...
	for(i=0; i<BLOOM_HASH_NUM; i++){
		pos=hash_i(i, elm)%(bf->bsz);
		bf->buf[pos>>3]|=(1<<(7-pos%8));
	}
}
	...
	long long elm /* the query element (a RK hash value) */ )
{
	...
	for(i=0; i<BLOOM_HASH_NUM; i++)
		if(!bloom_bit_at_pos(f, hash_i(i, elm) % f->bsz))
			return false;
