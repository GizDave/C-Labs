...
htable_init(htable *ht, int sz, int allow_resize) {
	...
	ht->mus = (pthread_mutex_t *)malloc(sizeof(pthread_mutex_t) * ht->size);
	for(int i=0; i<ht->size; i++){
		pthread_mutex_init(&ht->mus[i], NULL);
	}

	rw_lock = (rwl*) malloc(sizeof(rwl));
	//rwl_init(rw_lock);
	assert(ht->store);
...
htable_destroy(htable *ht) {
	...
	for (int i = 0; i < ht->size; i++){
		...
		pthread_mutex_destroy(&ht->mus[i]);
	}
	free(ht->store);
	//free(rw_lock);
}
...
htable_resize(htable *ht) {
	//printf("htable_resize()\n");
	rwl_wlock(rw_lock, NULL);
